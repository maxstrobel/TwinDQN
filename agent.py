...
class Agent(object):
    ...
    def select_action(self, state):
        ...
        if self.epsilon > random():
            ...
            state_variable = Variable(FloatTensor(torch.from_numpy(state))/255.0)
    ...
    def train(self):
        ...
        """
        save_self.net_episodes = 10000
        log_avg_episodes = 100
        num_episodes = 100000
        best_score = 0
        avg_score = 0
        target_net_updates = 0
        ...
        for i_episode in range(1, num_episodes):
            ...
            while not done:
                ...
                if len(self.replay) >= self.batch_size:
                    loss, reward_sum = self.optimize(target_net_updates)
                    target_net_updates += 1
            ...
            if i_episode % save_self.net_episodes == 0:
                self.net.save('dqn_' + str(i_episode) + '_episodes.self.net')
                
    def optimize(self, target_net_updates):
        """
        """
        ...
        # old version 
        #next_state_action_values[non_final_mask] = self.net(non_final_next_state_batch).max(1)[0] (old version)
        
        next_state_action_values[non_final_mask] = self.target_net(non_final_next_state_batch).detach().max(1)[0]
        ...
        # new version
        if target_net_updates % self.update_target_net_each_k == 0:
